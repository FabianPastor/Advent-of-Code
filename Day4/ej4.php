...
$passports_str = explode(PHP_EOL.PHP_EOL, $batch);
...
function isValid($passport){
  $fields_required = [
    "byr" => function($value){ return ( $value <= 2002 && $value >= 1920 ); },
    "iyr" => function($value){ return ( $value <= 2020 && $value >= 2010 ); },
    "eyr" => function($value){ return ( $value <= 2030 && $value >= 2020 ); },
    "hgt" => function($value){ 
  ...
  $result = array_diff_key($fields_required, $passport);
  if(!empty($result)) return false;
  
  foreach($fields_required as $field_name => $isValidField){
    if(!$isValidField($passport[$field_name])) return false;
  }
  return true;
}
